...
모든 실행 방식은 `env/secrets` 파일을 참조하며, Docker Compose의 RabbitMQ 값은 `.env` 파일에서도 읽습니다.
...
| `RABBITMQ_USERNAME`, `RABBITMQ_PASSWORD` | RabbitMQ 인증 정보 (env/secrets + `.env` 둘 다 일치하도록 설정) |
...
MetalLB가 할당한 IP는 `kubectl get svc -n ingress-nginx`로 확인할 수 있습니다. 
VM 내부에서 도메인 네임으로 접근하려면 `/etc/hosts`에 해당 IP ↔ 원하는 도메인(예: `approval.erp.local`) 매핑을 추가하거나, `curl -H "Host: employee.erp.local" http://<EXTERNAL-IP>/...` 방식으로 테스트하면 됩니다.
